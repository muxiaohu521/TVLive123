...
public class AudioTrackMemory {
    private static volatile AudioTrackMemory instance;
    ...
    public static AudioTrackMemory getInstance(Context context) {
        if (instance == null) {
            synchronized (AudioTrackMemory.class) {
                if (instance == null) {
                    instance = new AudioTrackMemory(context.getApplicationContext());
                }
            }
        }
        return instance;
